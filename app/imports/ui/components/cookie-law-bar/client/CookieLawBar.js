...
Template.CookieLawBar.onRendered(function() {
  let $cookiebar = jQuery('#cookie-bar');
  let $intercomLauncher = jQuery('#intercom-launcher');
  ...
  waitUntil(
    function() {
      return (
        jQuery('#intercom-launcher').is(':visible') &&
        jQuery('#cookie-bar').is(':visible')
      );
    },
    function() {
...
Template.CookieLawBar.events({
  'click .cb-enable': function(event) {
    let $cookiebar = jQuery(event.currentTarget).parent();
