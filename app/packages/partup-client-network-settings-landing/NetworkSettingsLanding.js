Template.NetworkSettingsLanding.onCreated(function() {
  ...
  this.subscribe('networks.one', slug, {
    onReady: () => {
      let network = Networks.findOne({ slug });
  ...
  this.onFileChange = (fileChangeEvent) => {
    Partup.client.uploader.eachFile(fileChangeEvent, (file) => {
      ...
      Partup.client.uploader.uploadImage(file, (error, image) => {
        ...
        if (error) {
          return Partup.client.notify.error(TAPi18n.__(error.reason));
        }
...
Template.NetworkSettingsLanding.helpers({
  form(...args) {
    ...
    const {
      name,
      content: { why_body, chat_body, about_body, ...restContent } = {},
    } =
      network || {};
    ...
    return {
      ...
      whyBodyInput: {
        ...
        placeholder: TAPi18n.__(
          'network-settings-landing-form-why-body-placeholder',
          { tribename: name }
        ),
        prefill: why_body,
      ...
      chatBodyInput: {
        ...
        className: 'pu-textarea pu-wysiwyg',
        placeholder: TAPi18n.__(
          'network-settings-landing-form-chat-body-placeholder',
          { tribename: name }
        ),
        prefill: chat_body,
        maxCharacters:
          Partup.schemas.forms.networkContent._schema.chat_body.max,
      ...
      aboutBodyInput: {
        ...
        className: 'pu-textarea pu-wysiwyg',
        placeholder: TAPi18n.__(
          'network-settings-landing-form-about-body-placeholder',
          { tribename: name }
        ),
        prefill: about_body,
        maxCharacters:
          Partup.schemas.forms.networkContent._schema.about_body.max,
  ...
  data(...args) {
    ...
    return {
      previewImage: () => {
        const imageId = lodash.get(
          network,
          'content.video_placeholder_image',
          imagePreviewId.get()
        );
        if (imageId) {
  ...
  placeholders(...args) {
    ...
    return {
      video_url: () =>
        TAPi18n.__('network-settings-landing-form-video-url-placeholder'),
      why_title: () =>
        TAPi18n.__('network-settings-landing-form-why-title-placeholder', {
          tribename: name,
        }),
      chat_title: () =>
        TAPi18n.__('network-settings-landing-form-chat-title-placeholder', {
          tribename: name,
        }),
      chat_subtitle: () =>
        TAPi18n.__('network-settings-landing-form-chat-subtitle-placeholder', {
          tribename: name,
        }),
      about_title: () =>
        TAPi18n.__('network-settings-landing-form-about-title-placeholder', {
          tribename: name,
        }),
    };
  },
