...
Template.Contribution.helpers({
  ...
  addsValue: function() {
    return (
      hasValue(this.contribution.hours) || hasValue(this.contribution.rate)
    );
  },
  showSplit: function() {
    return (
  ...
  canVerifyContribution: function() {
    ...
    let activity = Activities.findOne({
      _id: this.contribution.activity_id,
    });
    if (!activity) return false;
    let partup = Partups.findOne({ _id: activity.partup_id });
  ...
  rateTranslation: function() {
    let currency = this.contribution.currency || 'EUR';
...
Template.Contribution.events({
  ...
  'click [data-contribution-remove]': function(event, template) {
    Meteor.call(
      ...
      template.data.contribution._id,
      function(error) {
        if (error) {
          console.error(error);
        }
...
AutoForm.addHooks(null, {
  onSubmit: function(doc) {
    ...
    let template = this.template.parent();
