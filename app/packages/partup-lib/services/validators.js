...
Partup.services.validators = {
  ...
  // This only works for dutch phone numbers and has to be changed.
  phoneNumber: /(^\+[0-9]{2}|^\+[0-9]{2}\(0\)|^\(\+[0-9]{2}\)\(0\)|^00[0-9]{2}|^0)([0-9]{9}$|[0-9\-\s]{10}$)/,
  ...
  containsRequiredTags: function(string, requiredTags) {
    ...
    requiredTags.forEach(function(tag) {
      let matches = string.match(new RegExp('\\[\\s*' + tag + '\\s*\\]', 'i'));
  ...
  containsNoHtml: function(string) {
    ...
    let containsHtmlTags = new RegExp('<[a-z][\\s\\S]*>', 'i');
  ...
  containsNoUrls: function(string) {
    ...
    let containsUrls = new RegExp(
      /(["'])?(?:(?:[a-z]+:)?\/\/)(?:\S+(?::\S*)?@)?(?:localhost|(?:25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]|[0-9])(?:\.(?:25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]|[0-9])){3}|(?:(?:[a-z\u00a1-\uffff0-9]-*)*[a-z\u00a1-\uffff0-9]+)(?:\.(?:[a-z\u00a1-\uffff0-9]-*)*[a-z\u00a1-\uffff0-9]+)*(?:\.(?:[a-z\u00a1-\uffff]{2,})))(?::\d{2,5})?(?:\/\S*)?\1/gi
    );
  ...
  isVideoUrl: function(url) {
    ...
    let youtubeValid = url.match(youtubeRegex) ? RegExp.$1 : false;

    // Vimeo
    let vimeoRegex = /(?:https?:\/\/)?(?:www\.)?vimeo.com\/(?:channels\/(?:\w+\/)?|groups\/([^\/]*)\/videos\/|album\/(\d+)\/video\/|)(\d+)(?:$|\/|\?)/;
    let vimeoValid = vimeoRegex.test(url);
