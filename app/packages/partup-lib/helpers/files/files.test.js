...
Tinytest.add(
  'files.info[\'application/msword\'] returns FileInfo object',
  function(test) {
...
Tinytest.add(
  'FileInfo has category/mime/extension/signatures/icon',
  function() {
...
Tinytest.add(
  'FileInfo icon of docx falls back to category icon doc',
  function() {
...
Tinytest.add('getExtension returns extension from filename', function() {
  expect(Partup.helpers.files.getExtension({ name: 'filename.doc' })).to.equal(
    'doc'
  );
});

Tinytest.add('getExtension allows multiple dots in filename', function() {
  expect(Partup.helpers.files.getExtension({ name: 'file.name.doc' })).to.equal(
    'doc'
  );
});
...
Tinytest.add('getExtension returns extension from file mime', function() {
  expect(
    Partup.helpers.files.getExtension({ type: 'application/msword' })
  ).to.equal('doc');
});
...
Tinytest.add(
  'getCategory returns undefined when no category found',
  function() {
    expect(Partup.helpers.files.getCategory('someextension')).to.equal(
      undefined
    );
  }
);
...
Tinytest.add('isImage returns false when mime is not image', function() {
  expect(Partup.helpers.files.isImage({ type: 'application/msword' })).to.equal(
    false
  );
});

Tinytest.add(
  'isImage returns true when valid mime and invalid name',
  function() {
    expect(
      Partup.helpers.files.isImage({
        name: 'invalid',
        type: 'image/jpeg',
      })
    ).to.equal(true);
...
Tinytest.add(
  'isImage returns true when valid name and invalid mime',
  function() {
    expect(
      Partup.helpers.files.isImage({ name: 'image.jpg', type: 'invalid' })
    ).to.equal(true);
  }
);
...
Tinytest.add('getSvgIcon returns icon when FileInfo has icon', function() {
  expect(
    Partup.helpers.files.getSvgIcon({
      name: 'filename.doc',
      type: 'application/msword',
    })
  ).to.equal('doc.svg');
...
Tinytest.add(
  'getSvgIcon returns icon when multiple FileInfo objects found',
  function() {
    expect(
      Partup.helpers.files.getSvgIcon({ type: 'application/gzip' })
    ).to.equal('zip.svg');
  }
);
...
Tinytest.add('toUploadFilter returns filter for category images', function(
  test
) {
  test.isNotUndefined(
    Partup.helpers.files.toUploadFilter(Partup.helpers.files.categories.image)
  );
});
...
Tinytest.add('toUploadFilter allows custom filter', function() {
  ...
  expect(
    Partup.helpers.files.toUploadFilter('custom', extensions).title
  ).to.equal('custom');
  expect(
    Partup.helpers.files.toUploadFilter('custom', extensions).extensions
  ).to.equal(extensions);
});
...
Tinytest.add(
  'shortToBinarySize returns binary size on size in bytes',
  function() {
...
Tinytest.add(
  'shortToBinarySize returns binary size on size in kilobytes',
  function() {
