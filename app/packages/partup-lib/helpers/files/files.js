...
Partup.helpers.files = {
  ...
  getExtension(file) {
    ...
    if (info) {
      return info[0] ? undefined : info.extension;
    }
    return undefined;
  },
  ...
  getCategory(file) {
    ...
    if (info) {
      return info[0] ? info[0].category : info.category;
    }
    return undefined;
  },
  ...
  isImage(file) {
    ...
    if (info) {
      return info[0]
        ...
        : _.includes(this.extensions.image, info.extension) ||
            info.category === this.categories.image;
    }
    return false;
  },
  ...
  shortToBinarySize(size) {
    if (!(typeof size === 'string')) {
      throw new Meteor.Error(
        0,
        `expected size to be of type string but is ${typeof size}`
      );
    }
...
_.each(_filemap, ({ category, icon, data }) => {
  ...
  _.each(data, (fileInfo) => {
    ...
    Partup.helpers.files.info[mime]
      ? (Partup.helpers.files.info[mime] = _.concat(
        Partup.helpers.files.info[mime],
        [info]
      ))
 ...
 */
Partup.helpers.files.extensions.all = _.reduce(
  Object.keys(Partup.helpers.files.extensions),
  (result, key) => {
 ...
 * */
Partup.helpers.files.categories.all = _.reduce(
  Object.keys(Partup.helpers.files.categories),
  (result, key) => {
  ...
  Partup.helpers.files.transform = {
    dropbox(dropboxFile) {
      const file = {
        ...
        type:
          Partup.helpers.files.info[
            Partup.helpers.files.getExtension(dropboxFile)
          ].mime,
