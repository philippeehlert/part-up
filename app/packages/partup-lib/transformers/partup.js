...
Partup.transformers.partup = {
  ...
  toFormStartPartup: function(partup) {
    ...
    let fields = {
      ...
      location_input: Partup.services.location.locationToLocationInput(
        partup.location
      ),
      name: partup.name,
      tags_input: Partup.services.tags.tagArrayToInput(partup.tags),
      focuspoint_x_input: image
        ? mout.object.get(image, 'focuspoint.x') || 0
        : 0,
      focuspoint_y_input: image
        ? mout.object.get(image, 'focuspoint.y') || 0
        : 0,
      phase: partup.phase,
  ...
  fromFormStartPartup: function(fields) {
    let partup = {
      ...
      language: Partup.server.services.google.detectLanguage(
        fields.description
      ),
      phase: fields.phase,
    ...
    let newLocation = Partup.services.location.locationInputToLocation(
      fields.location_input
    );
    if (newLocation) partup.location = newLocation;

    // Set network ID if needed
    if (
      fields.privacy_type_input === 'network' ||
      fields.privacy_type_input === 'network_admins' ||
      fields.privacy_type_input === 'network_colleagues' ||
      fields.privacy_type_input === 'network_colleagues_custom_a' ||
      fields.privacy_type_input === 'network_colleagues_custom_b'
    ...
    // Save focuspoint
    Partup.server.services.images.storeFocuspoint(
      partup.image,
      fields.focuspoint_x_input || 0,
      fields.focuspoint_y_input || 0
    );
