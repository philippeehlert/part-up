...
Template.ImageSystem.onCreated(function() {
  if (!this.data.imageId) {
    throw new Error(
      'The ImageSystem is dependant on \'imageId\', this has to be a ReactiveVar passed in from it\'s parent.'
    );
  }
...
Template.ImageSystem.onRendered(function() {
  ...
  template.uploader = new Pluploader({
    ...
    types: [Partup.helpers.files.categories.image],
    hooks: {
        ...
        _.each(files, (file) => {
          if (!Partup.helpers.files.isImage(file)) {
            ...
            Partup.client.notify.info(
              TAPi18n.__('upload-error-not_image', {
                filename: file.name,
              })
            );
          }
      ...
      FileUploaded(uploader, file, result) {
        ...
        if (!response.fileId) {
          template.loading.set(false);
          Partup.client.notify.info(TAPi18n.__('upload-error-100'), {
            filename: file.name,
          });
          return;
        }
        ...
        template.imageSubHandler = template.subscribe(
          'images.one',
          response.fileId,
          {
            onReady() {
