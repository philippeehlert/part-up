Template.NetworkNavigationSelector.onCreated(function() {
  ...
  template.dropdownToggleBool =
    'partial-dropdowns-networks-actions-navigation.opened';
  template.dropdownOpen = new ReactiveVar(false);

  let networkSlug = template.data.slug;
  ...
  if (template.data.default === 'network-detail') {
    defaultOption = template.options[0];
  }
  if (template.data.default === 'network-chat') {
    defaultOption = template.options[1];
  }
  if (template.data.default === 'network-uppers') {
    defaultOption = template.options[2];
  }
  if (template.data.default === 'network-about') {
