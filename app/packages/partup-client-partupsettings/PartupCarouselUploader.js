...
Template.PartupCarouselUploader.onCreated(function() {
  ...
  if (template.data.currentPartup && template.data.currentPartup.highlighted) {
    template.items = new ReactiveVar([
      ...template.data.currentPartup.highlighted,
    ]);
  } else {
    template.items = new ReactiveVar([]);
  }
      ...
      onFileChange: (event) => {
        ...
        if (currentItems.length >= maxItems) {
          Partup.client.notify.error(
            'Cannot add more than 4 photos or videos, please remove one first.'
          );
          return;
        }
        ...
        Partup.client.uploader.eachFile(event, function(file, index, type) {
          ...
          Partup.client.uploader.uploadImage(file, function(error, image) {
            if (error) {
              Partup.client.notify.error(
                TAPi18n.__('profilesettings-form-image-error')
              );
              template.uploadingPhoto.set(false);
              return;
            }
...
Template.PartupCarouselUploader.events({
  ...
  'click [data-add-video-url]': (event, template) => {
    ...
    if (!isYoutube && !isVimeo) {
      Partup.client.notify.error(
        'Video url needs to be a valid youtube video url.'
      );
      return;
    }
    ...
    const videoObject = {
      ...
      thumb: isYoutube
        ...
        : `https://i.vimeocdn.com/video/${videoId}_640.jpg`,
      url: value,
    };

    template.items.set([...template.items.get(), videoObject]);
