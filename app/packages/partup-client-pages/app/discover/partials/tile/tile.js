...
Template.PartupTile.onCreated(function() {
  ...
  partup.imageObject =
    partup.imageObject || Images.findOne({ _id: partup.image });
  partup.boundedProgress = partup.progress
    ? Math.max(10, Math.min(99, partup.progress))
    : 10;

  // -- Partup network
  partup.networkObject =
    partup.networkObject || Networks.findOne({ _id: partup.network_id });
  if (partup.networkObject) {
    partup.networkObject.iconObject =
      partup.networkObject.iconObject ||
      Images.findOne({ _id: partup.networkObject.icon });
    ...
    partup.networkObject.userIsDefinitlyMember = Partup.client.partuptile.userIsDefinitlyMemberOfNetwork(
      partup.networkObject,
      Meteor.userId()
    );
  }
  ...
  partup.avatars = partup.uppers
    ...
    .map((avatar, index, arr) => {
      // Avatar position
      const default_coords = getAvatarCoordinates(
        arr.length,
        index,
        0,
        AVATAR_DEFAULT_DISTANCE,
        AVATAR_DEFAULT_RADIUS
      );
      const hovering_coords = getAvatarCoordinates(
        arr.length,
        index,
        0,
        AVATAR_HOVERING_DISTANCE,
        AVATAR_HOVERING_RADIUS
      );
      const position = {
        default: {
          delay: 0.03 * index,
      ...
      const upper =
        lodash.find(uppers, { _id: avatar }) ||
        Meteor.users.findOne({ _id: avatar });
      upper.avatarObject = lodash.get(
        upper,
        'profile.imageObject',
        Images.findOne({ _id: lodash.get(upper, 'profile.image') })
      );
...
Template.PartupTile.onRendered(function() {
  ...
  if (partup.image && !partup.archived_at) {
    const {
      imageObject: image = Images.findOne({ _id: partup.image }),
    } = partup;
...
Template.PartupTile.helpers({
  ...
  showStartQuery: function() {
    return this.userIsDefinitlyMember
      ? this.archived_at ? 'show=true' : 'show=false'
      : '';
  },
  ...
  shortLabel: function(partup) {
    ...
    if (privacyTypeLabels && privacyTypeLabels[partup.privacy_type]) {
      ...
      return TAPi18n.__('partup-tile-label-custom-short', {
        name: privacyTypeLabels[partup.privacy_type],
      });
    }
    if (partup.privacy_type === Partups.privacy_types.NETWORK_ADMINS) {
      return TAPi18n.__('partup-tile-label-admins-only-short');
    }
    if (partup.privacy_type === Partups.privacy_types.NETWORK_COLLEAGUES) {
      return TAPi18n.__('partup-tile-label-colleagues-only-short');
    }
    if (
      partup.privacy_type === Partups.privacy_types.NETWORK_COLLEAGUES_CUSTOM_A
    ) {
      return TAPi18n.__('partup-tile-label-colleagues-custom-a-only-short');
    }
    if (
      partup.privacy_type === Partups.privacy_types.NETWORK_COLLEAGUES_CUSTOM_B
    ) {
      return TAPi18n.__('partup-tile-label-colleagues-custom-b-only-short');
    }
    return undefined;
  },
  longLabel: function(partup) {
    ...
    if (privacyTypeLabels && privacyTypeLabels[partup.privacy_type]) {
      if (partup.privacy_type === 6) {
        return TAPi18n.__('partup-tile-label-admin-custom', {
          name: privacyTypeLabels[partup.privacy_type],
        });
      }
      return TAPi18n.__('partup-tile-label-custom', {
        name: privacyTypeLabels[partup.privacy_type],
      });
    }
    if (partup.privacy_type === Partups.privacy_types.NETWORK_ADMINS) {
      return TAPi18n.__('partup-tile-label-admins-only');
    }
    if (partup.privacy_type === Partups.privacy_types.NETWORK_COLLEAGUES) {
      return TAPi18n.__('partup-tile-label-colleagues-only');
    }
    if (
      partup.privacy_type === Partups.privacy_types.NETWORK_COLLEAGUES_CUSTOM_A
    ) {
      return TAPi18n.__('partup-tile-label-colleagues-custom-a-only');
    }
    if (
      partup.privacy_type === Partups.privacy_types.NETWORK_COLLEAGUES_CUSTOM_B
    ) {
      return TAPi18n.__('partup-tile-label-colleagues-custom-b-only');
    }
    return undefined;
  },
});
