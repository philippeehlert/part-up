  ...
  if (this.images.length) {
    ...
    setTimeout(() => {
      $('.pu-carousel').slick({
        ...
        arrows: true,
        prevArrow:
          '<button type="button" class="pu-carousel-prev"><i class="picon-caret-slim-left"></i></button>',
        nextArrow:
          '<button type="button" class="pu-carousel-next"><i class="picon-caret-slim-right"></i></button>',
        autoplay: true,
        autoplaySpeed: 10000,
        onBeforeChange: (carousel, currentIndex) => {
          ...
          if (current && current.type === 'video') {
            template
              .$(`[data-video="${currentIndex}"] .video-layer`)
              .removeClass('hide');
            template
              .$(`[data-video="${currentIndex}"] iframe`)
              .each((index, video) => {
      ...
      $('.pu-carousel-prev, .pu-carousel-next, .slick-dots li').on(
        'click',
        function() {
