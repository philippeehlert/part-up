...
Template.app_partup_navigation.helpers({
  ...
  isPartnerOrSupporter(user) {
    ...
    if (partup) {
      return (
        User(user).isSupporterInPartup(partup._id) ||
        User(user).isPartnerInPartup(partup._id)
      );
    }
  },
});
...
Template.app_partup_navigation.events({
  ...
  'click [data-end-partup-partnership]': function(event, template) {
    ...
    Partup.client.prompt.confirm({
      title: TAPi18n.__('pages-app-partup-popup-title-unpartner', {
        partup: template.data.partup.name,
      }),
  ...
  'click [data-share-facebook]': function(event, template) {
    const currentUrl = Router.url('partup', {
      slug: template.data.partup.slug,
    });
  ...
  'click [data-share-twitter]': function(event, template) {
    ...
    const shareUrl = Partup.client.socials.generateTwitterShareUrl(
      template.data.partup.name,
      currentUrl
    );
  ...
  'click [data-share-mail]': function(event, template) {
    ...
    const body = user
      ? TAPi18n.__('pages-app-partup-share_mail', {
        url: currentUrl,
        partup_name: template.data.partup.name,
        user_name: user.profile.name,
      })
      : TAPi18n.__('pages-app-partup-share_mail_anonymous', {
        url: currentUrl,
        partup_name: template.data.partup.name,
      });
