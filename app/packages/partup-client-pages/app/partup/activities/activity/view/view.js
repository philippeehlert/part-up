...
Template.ActivityView.onCreated(function() {
  ...
  this.activityId = template.data.activity
    ? template.data.activity._id
    : template.data.activity_id;
  ...
  template.expanded = new ReactiveVar(
    !!template.data.EXPANDED || !!template.data.CREATE_PARTUP
  );
  ...
  this.update = Updates.findOne({
    _id:
      this.data.updateId || get(Template.instance(), 'data.activity.update_id'),
  });
  template.hidden = {
    comments: new ReactiveVar(
      template.data.BOARDVIEW ||
        (template.data.EXPANDED && !_.get(this.update, 'comments_count'))
    ),
...
Template.ActivityView.helpers({
  ...
  truncateDescription() {
    return function(text) {
      ...
      const htmlText = strings.renderToMarkdownWithEmoji(
        text,
        'pu-sub-description'
      );
  ...
  showContributeButton: function() {
    ...
    let contributions = Contributions.findForActivity(this.activity).fetch();
    for (let i = 0; i < contributions.length; i++) {
      if (
        contributions[i].upper_id === user._id &&
        !contributions[i].archived
      ) {
        return false;
      }
    }
  ...
  update: function() {
    return Updates.findOne({
      _id: this.updateId || get(Template.instance(), 'data.activity.update_id'),
    });
  },
  popupId: function() {
    return (
      'popup.motivation.' +
      (this.updateId || get(Template.instance(), 'data.activity.update_id'))
    );
  },
    ...
    return {
      ...
      isUpper() {
        return User(Meteor.user()).isPartnerInPartup(
          get(self.activity, 'partup_id') || self.partupId
        );
      },
      updateIsStarred() {
...
Template.ActivityView.events({
  'click [data-detail]'(event, templateInstance) {
    ...
    const partup = Partups.findOne({
      _id: templateInstance.data.activity.partup_id,
    });
    const detailIsFiles = $($(event.target).closest('[data-detail]')).data(
      'detail'
    );

    if (
      templateInstance.data.BOARDVIEW ||
      (templateInstance.data.EXPANDABLE && !templateInstance.expanded.get())
    ) {
      Router.go(
        'partup-update',
        {
          slug: partup.slug,
          update_id:
            templateInstance.data.updateId ||
            templateInstance.data.activity.update_id,
        },
        {
  ...
  'click [data-activity-expander]': function(event, template) {
    let partup = Partups.findOne({ _id: template.data.activity.partup_id });
    ...
    let opened = template.expanded.get();
    template.expanded.set(!opened);
  },
  'click [data-contribute]': function(event, template) {
    event.preventDefault();

    let contribute = function() {
      let partup = Partups.findOne({
        _id: template.data.activity.partup_id,
      });

      if (!partup) {
        Partup.client.notify.error(
          'Couldn\'t proceed your contribution. Please try again!'
        );
        return;
      }
      ...
      if (!partup.hasUpper(Meteor.userId())) {
        let popupId =
          'popup.motivation.' +
          (template.data.updateId || template.data.activity.update_id);
        Partup.client.popup.open(
          {
            id: popupId,
          },
  ...
  'click [data-archive]': (event, template) => {
    Meteor.call('activities.archive', template.data.activity._id, function(
      error
    ) {
      if (error) {
  ...
  'click [data-unarchive]': (event, template) => {
    Meteor.call('activities.unarchive', template.data.activity._id, function(
      error
    ) {
      if (error) {
        Partup.client.notify.error(error.reason);
    ...
    Meteor.call('updates.messages.star', updateId, function(error, result) {
      if (error) {
        ...
        if (error.reason === 'partup_message_too_many_stars') {
          Partup.client.notify.error(
            TAPi18n.__('pur-partup-start-error-too_many_starred_updates')
          );
        } else {
