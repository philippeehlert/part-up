Template.Start_Video.helpers({
  data(...args) {
    ...
    const {
      name: tribename,
      content: { video_url, video_placeholder_image, why_title, why_body } = {},
    } = Networks.findOne({ slug });
    return {
      video_url: () =>
        video_url ||
        TAPi18n.__('pages-app-network-landing-intro-video-fallback'),
      video_placeholder: () => {
      ...
      why_title: () =>
        why_title ||
        TAPi18n.__('pages-app-network-landing-intro-why-title-fallback', {
          tribename,
        }),
      why_body: () =>
        why_body ||
        TAPi18n.__('pages-app-network-landing-intro-why-body-fallback', {
          tribename,
        }),
    };
  },
