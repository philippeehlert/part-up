...
Template.app_network.helpers({
  data: function() {
    ...
    let networkSlug = template.networkSlug.get();
    ...
    return {
      ...
      unreadChatMessages: function() {
        ...
        let chat = Chats.findOne({ _id: network.chat_id });
...
let leaveNetwork = function(template, network) {
  Meteor.call('networks.leave', network._id, function(error) {
    ...
    Partup.client.notify.success(
      TAPi18n.__('pages-app-network-notification-left')
    );
...
Template.app_network.events({
  ...
  'click [data-location]': function(event, template) {
    event.preventDefault();
    let networkSlug = template.networkSlug.get();
    let location = Networks.findOne({ slug: networkSlug }).location;
  ...
  'click [data-header-navigate]': function(event, template) {
    Router.go(
      'network',
      { slug: template.networkSlug.get() },
      { query: 'show=true' }
    );
  },
  'click [data-leave-tribe]': function(event, template) {
    event.preventDefault();
    let network = Networks.findOne({ slug: template.networkSlug.get() });
    Partup.client.prompt.confirm({
      ...
      confirmButton: TAPi18n.__(
        'pages-app-network-confirmation-confirm-button'
      ),
