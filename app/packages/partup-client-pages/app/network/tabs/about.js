Template.app_network_about.onCreated(function() {
    ...
    this.contentSubscription = this.subscribe('contentblocks.by_network_slug', networkSlug);
    this.networkSubscription = this.subscribe('admins.by_network_slug', networkSlug);

    this.autorun((computation) => {
        const ready = !!(
            this.contentSubscription.ready() &&
            this.networkSubscription.ready()
        );
        if (!ready) return;
        const network = Networks.findOne({slug: networkSlug});
        const { contentblocks: contentBlockIds = [], tags, website, location } = network;

        const introBlock = ContentBlocks
            .findOne({
        ...
        const contentBlocks = ContentBlocks
            ...
            .fetch()
            .sort((a, b) => {
                return contentBlockIds.indexOf(a._id) > contentBlockIds.indexOf(b._id) ? 1 : -1;
            });
...
Template.app_network_about.helpers({
    state(...args) {
        ...
        return {
            loaded: () => (
                contentSubscription.ready() &&
                networkSubscription.ready()
            ),
        };
    },
    data(...args) {
        const { data: { networkSlug }, introBlock, contentBlocks } = Template.instance();
        const network = Networks.findOne({slug: networkSlug});
