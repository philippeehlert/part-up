...
Template.modal_swarm_settings_tribes.helpers({
  ...
  form: function() {
    let template = Template.instance();
    let swarmSlug = template.data.slug;
    return {
      ...
      networkQuery: function() {
        return function(query, sync, async) {
          Meteor.call('networks.autocomplete_swarm', query, swarmSlug, function(
            error,
            networks
          ) {
  ...
  'click [data-removenetwork]': function(event, template) {
    ...
    Meteor.call('swarms.remove_network', swarm._id, networkId, function(err) {
      ...
      Partup.client.notify.success(
        TAPi18n.__('modal-swarm-tribes-remove-success')
      );
    });
  },
});
...
AutoForm.addHooks('addNetworkForm', {
  onSubmit: function(doc) {
    ...
    let networkId = template.networkSelection.curValue._id;
    let swarm = Swarms.findOne({ slug: template.data.slug });
    Meteor.call('swarms.add_network', swarm._id, networkId, function(err) {
      ...
      Partup.client.notify.success(
        TAPi18n.__('modal-swarm-tribes-form-success')
      );
