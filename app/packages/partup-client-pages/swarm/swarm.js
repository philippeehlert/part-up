Template.swarm.onCreated(function() {
  ...
  Meteor.call('swarms.get_related_networks', template.data.slug, function(
    error,
    res
  ) {
  ...
  Meteor.call('swarms.get_related_uppers', template.data.slug, function(
    error,
    res
  ) {
    let userIds = mout.array.map(res, function(user) {
      return user._id;
    });
    ...
    return {
      ...
      relatedNetworks: function() {
        return Networks.find({
          _id: { $in: template.relatedNetworks.get() },
        });
      },
