  ...
  data: function() {
    ...
    return {
      rings: function() {
        ...
        let networks =
          data.networks instanceof Mongo.Collection.Cursor
            ? data.networks.fetch()
            : false;
        let innerRingItems = [];
        let outerRingItems = [];
        ...
        let empty = total ? false : true;
        if (total > template.pageLimit) {
          let totalPages = Math.ceil(total / template.pageLimit);
          var total = Math.ceil(total / totalPages);
          var numberOfInnerRingItems = Math.min(
            3,
            Math.max(Math.ceil(total / 3), 2)
          );
