  ...
  $.fn.clipboard = function(params) {
    if (
      ...
      return this.each(function() {
        ...
        if (
          o.is(':visible') &&
          (typeof settings.copy == 'string' || $.isFunction(settings.copy))
        ) {
...
(function() {
  ...
  let _getStyle = function(el, prop) {
    let value, camelProp, tagName, possiblePointers, i, len;
  ...
  let _getDOMObjectPosition = function(obj) {
    let info = {
      left: 0,
      top: 0,
      width: 0,
  ...
  let _noCache = function(path, options) {
    let useNoCache = !(options && options.useNoCache === false);
    if (useNoCache) {
      return (
        (path.indexOf('?') === -1 ? '?' : '&') +
        'nocache=' +
  ...
  let _inArray = function(elem, array) {
    if (array.indexOf) {
  ...
  let _prepGlue = function(elements) {
  ...
  let _dispatchCallback = function(func, element, instance, args, async) {
    if (async) {
  ...
  var ZeroClipboard = function(elements, options) {
    ...
    this.options = {};
    for (let kd in _defaults) this.options[kd] = _defaults[kd];
    for (let ko in options) this.options[ko] = options[ko];
    this.handlers = {};
  ...
  ZeroClipboard.prototype.setCurrent = function(element) {
    ...
    this.reposition();
    let titleAttr = element.getAttribute('title');
    if (titleAttr) {
  ...
  ZeroClipboard.setDefaults = function(options) {
    for (let ko in options) _defaults[ko] = options[ko];
  };
  ZeroClipboard.destroy = function() {
    if (ZeroClipboard.prototype._singleton) {
      ZeroClipboard.prototype._singleton.unglue(gluedElements);
      let bridge = ZeroClipboard.prototype._singleton.htmlBridge;
  ...
  let _amdModuleId = null;
  let _cjsModuleId = null;
  var _bridge = function() {
    let flashBridge, len;
    let client = ZeroClipboard.prototype._singleton;
    let container = document.getElementById('global-zeroclipboard-html-bridge');
    if (!container) {
      let opts = {};
      for (let ko in client.options) opts[ko] = client.options[ko];
      opts.amdModuleId = _amdModuleId;
      opts.cjsModuleId = _cjsModuleId;
      let flashvars = _vars(opts);
      let html =
  ...
  ZeroClipboard.prototype.reposition = function() {
    ...
    let pos = _getDOMObjectPosition(currentElement);
  ...
  ZeroClipboard.prototype.on = function(eventName, func) {
    let events = eventName.toString().split(/\s/g),
      added = {};
  ...
  ZeroClipboard.prototype.receiveEvent = function(eventName, args) {
    ...
    switch (eventName) {
      ...
      case 'datarequested':
        ...
        if (targetEl) {
          ...
          let defaultText = element.getAttribute('data-clipboard-text');
          if (defaultText) {
    ...
    if (this.handlers[eventName]) {
      let func = this.handlers[eventName];
  ...
  ZeroClipboard.prototype.unglue = function(elements) {
    ...
    for (let i = 0; i < elements.length; i++) {
      ...
      let arrayIndex = _inArray(elements[i], gluedElements);
