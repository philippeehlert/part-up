...
Publication.prototype.publish = function publish() {
  ...
  let collectionName = this._getCollectionName();
  let self = this;

  this.observeHandle = this.cursor.observe({
    added: function added(doc) {
      let alreadyPublished = self.publishedDocs.has(doc._id);

      if (alreadyPublished) {
        debugLog(
          'Publication.observeHandle.added',
          collectionName + ':' + doc._id + ' already published'
        );
    ...
    changed: function changed(newDoc) {
      debugLog(
        'Publication.observeHandle.changed',
        collectionName + ':' + newDoc._id
      );
      self._republishChildrenOf(newDoc);
    },
    removed: function removed(doc) {
      debugLog(
        'Publication.observeHandle.removed',
        collectionName + ':' + doc._id
      );
  ...
  this.observeChangesHandle = this.cursor.observeChanges({
    changed: function changed(id, fields) {
      debugLog(
        'Publication.observeChangesHandle.changed',
        collectionName + ':' + id
      );
...
Publication.prototype._getCollectionName = function _getCollectionName() {
  return (
    this.collectionName || (this.cursor && this.cursor._getCollectionName())
  );
};
...
Publication.prototype._republishChildrenOf = function _republishChildrenOf(
  doc
) {
...
Publication.prototype._unpublishChildrenOf = function _unpublishChildrenOf(
  docId
) {
  debugLog(
    'Publication._unpublishChildrenOf',
    'unpublishing children of ' + this._getCollectionName() + ':' + docId
  );
...
PublishedDocumentList.prototype.addChildPub = function addChildPub(
  docId,
  publication
) {
  if (!publication) {
...
PublishedDocumentList.prototype.eachDocument = function eachDocument(
  callback,
  context
) {
  _.each(
...
PublishedDocumentList.prototype.eachChildPub = function eachChildPub(
  docId,
  callback
) {
...
PublishedDocumentList.prototype.unflagForRemoval = function unflagForRemoval(
  docId
) {
  let doc = this.get(docId);
...
PublishedDocument.prototype.addChildPub = function addChildPub(
  childPublication
) {
...
PublishedDocument.prototype.eachChildPub = function eachChildPub(callback) {
  for (let i = 0; i < this.childPublications.length; i++) {
