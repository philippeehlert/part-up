Subscription = function(meteorSub) {
  ...
  this.refCounter = new DocumentRefCounter({
    onChange: function(collectionName, docId, refCount) {
      debugLog(
        'Subscription.refCounter.onChange',
        collectionName + ':' + docId.valueOf() + ' ' + refCount
      );
      if (refCount <= 0) {
...
Subscription.prototype._updateDocHash = function(collectionName, id, changes) {
  let key = this._buildHashKey(collectionName, id);
  let existingDoc = this.docHash[key] || {};
...
Subscription.prototype._shouldSendChanges = function(
  ...
  return (
    this._isDocPublished(collectionName, id) &&
...
Subscription.prototype._hasDocChanged = function(collectionName, id, doc) {
  let existingDoc = this.docHash[this._buildHashKey(collectionName, id)];
