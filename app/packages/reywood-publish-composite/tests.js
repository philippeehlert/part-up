 ...
 * Define test helper
 */
let testPublication = function(testName, options) {
  options.args = options.args || [];

  Tinytest.addAsync(testName, function(assert, onComplete) {
    let subscription;
    let args = [options.publication].concat(options.args);
...
if (Meteor.isClient) {
  ...
  testPublication('Should publish all post authors', {
    ...
    testHandler: function(assert, onComplete) {
      ...
      posts.forEach(function(post) {
        let author = Authors.findOne({ username: post.author });
  ...
  testPublication('Should publish all post comment authors', {
    ...
    testHandler: function(assert, onComplete) {
      ...
      comments.forEach(function(comment) {
        let author = Authors.findOne({ username: comment.author });
  ...
  testPublication('Should publish one user\'s posts', {
    ...
    testHandler: function(assert, onComplete) {
      let allSubscribedPosts = Posts.find();
      assert.equal(allSubscribedPosts.count(), 2, 'Post count');

      let postsByOtherAuthors = Posts.find({ author: { $ne: 'marie' } });
  ...
  testPublication('Should remove author when comment is deleted', {
    ...
    testHandler: function(assert, onComplete) {
      let mariesSecondPost = Posts.findOne({
        title: 'Marie\'s second post',
      });

      assert.equal(
        Authors.find({ username: 'richard' }).count(),
        1,
        'Author present pre-delete'
      );

      let richardsComment = Comments.findOne({
        postId: mariesSecondPost._id,
        author: 'richard',
      });
  ...
  testPublication(
    'Should not remove author when comment is deleted if author record still needed',
    {
      publication: 'userPosts',
      args: ['marie'],
      ...
      testHandler: function(assert, onComplete) {
        ...
        assert.equal(
          Authors.find({ username: 'marie' }).count(),
          1,
          'Author present pre-delete'
        );

        let mariesComment = Comments.findOne({
          postId: mariesSecondPost._id,
          author: 'marie',
        });
        ...
        Meteor.call('removeComment', mariesComment._id, function(err) {
          ...
          assert.equal(
            Authors.find({ username: 'marie' }).count(),
            1,
            'Author still present post-delete'
          );
  ...
  testPublication(
    'Should remove both post and author if post author is changed',
    {
      publication: 'userPosts',
      args: ['stephen'],

      testHandler: function(assert, onComplete) {
        let post = Posts.findOne({ title: 'Post with no comments' });

        assert.isTrue(typeof post !== 'undefined', 'Post present pre-change');
        assert.equal(
          Authors.find({ username: 'stephen' }).count(),
          1,
          'Author present pre-change'
        );
  ...
  testPublication(
    'Should publish new author and remove old if comment author is changed',
    {
      publication: 'userPosts',
      args: ['albert'],

      testHandler: function(assert, onComplete) {
        let albertsPost = Posts.findOne({
          title: 'Post with one comment',
        });
        let comment = Comments.findOne({
          postId: albertsPost._id,
          author: 'richard',
        });
        ...
        assert.equal(
          Authors.find({ username: 'richard' }).count(),
          1,
          'Old author present pre-change'
        );
        assert.equal(
          Authors.find({ username: 'john' }).count(),
          0,
          'New author absent pre-change'
        );
        ...
        Meteor.call('updateCommentAuthor', comment._id, 'john', function(err) {
          ...
          assert.equal(
            Authors.find({ username: 'richard' }).count(),
            0,
            'Old author absent post-change'
          );
          assert.equal(
            Authors.find({ username: 'john' }).count(),
            1,
            'New author present post-change'
          );
  ...
  testPublication(
    'Should remove post, comment, and comment author if post is deleted',
    {
      publication: 'userPosts',
      args: ['marie'],

      testHandler: function(assert, onComplete) {
        let mariesFirstPost = Posts.findOne({
          title: 'Marie\'s first post',
        });

        assert.isTrue(
          typeof mariesFirstPost !== 'undefined',
          'Post present pre-change'
        );
        assert.equal(
          Comments.find({
            postId: mariesFirstPost._id,
            author: 'albert',
          }).count(),
          1,
          'Comment present pre-change'
        );
        assert.equal(
          Authors.find({ username: 'albert' }).count(),
          1,
          'Comment author present pre-change'
        );
        ...
        Meteor.call('removePost', mariesFirstPost._id, function(err) {
          ...
          assert.equal(
            Posts.find({ title: 'Marie\'s first post' }).count(),
            0,
            'Post absent post-change'
          );
          assert.equal(
            Comments.find({
              postId: mariesFirstPost._id,
            ...
            0,
            'Comment absent post-change'
          );
          assert.equal(
            Authors.find({ username: 'albert' }).count(),
            0,
            'Comment author absent post-change'
          );
  ...
  testPublication(
    'Should publish posts to client side collection named "articles"',
    {
      ...
      testHandler: function(assert, onComplete) {
        assert.equal(
          Posts.find().count(),
          0,
          'Posts collection empty on client'
        );
        assert.equal(
          Articles.find().count(),
          4,
          'Articles collection not empty on client'
        );
  ...
  testPublication(
    'Should handle going from null cursor to non-null cursor when republishing',
    {
      ...
      testHandler: function(assert, onComplete) {
        ...
        let comments = Comments.find({ postId: mariesFirstPost._id });

        assert.isTrue(comments.count() === 0, 'No comments published');

        Meteor.call('updatePostAuthor', mariesFirstPost._id, 'albert', function(
          err
        ) {
          assert.isUndefined(err);
  ...
  testPublication(
    'Should handle going from non-null cursor to null cursor when republishing',
    {
      publication: 'pubWithChildThatReturnsNullIfAuthorIsMarie',

      testHandler: function(assert, onComplete) {
        let albertsPost = Posts.findOne({ author: 'albert' });
        let comments = Comments.find({ postId: albertsPost._id });

        assert.isTrue(comments.count() > 0, 'Comments published');

        Meteor.call('updatePostAuthor', albertsPost._id, 'marie', function(
          err
        ) {
          assert.isUndefined(err);
  ...
  testPublication('Should remove field from document when it is unset', {
    ...
    testHandler: function(assert, onComplete) {
      let albertsPost = Posts.findOne({ author: 'albert' });
      let comment = Comments.findOne({ postId: albertsPost._id });

      assert.isTrue(
        typeof comment.text !== 'undefined',
        'Comment has text field'
      );
      ...
      Meteor.call('unsetCommentText', comment._id, function(err) {
        ...
        assert.isTrue(
          typeof comment.text === 'undefined',
          'Comment no longer has text field'
        );
  ...
  testPublication(
    'Should publish authors to both Authors and CommentAuthors collections',
    {
      publication: 'publishCommentAuthorsInAltClientCollection',

      testHandler: function(assert, onComplete) {
        let albertAsAuthor = Authors.findOne({ username: 'albert' });
        let albertAsCommentAuthor = CommentAuthors.findOne({
          username: 'albert',
        });

        assert.isTrue(
          typeof albertAsAuthor !== 'undefined',
          'Albert present in Authors collection'
        );
        assert.isTrue(
          typeof albertAsCommentAuthor !== 'undefined',
          'Albert present in CommentAuthors collection'
        );
  ...
  testPublication(
    'Should publish two top level publications specified with a function',
    {
      publication: 'twoUsersPosts',
      args: ['marie', 'albert'],

      testHandler: function(assert, onComplete) {
        let mariesPost = Posts.findOne({ author: 'marie' });
        let albertsPost = Posts.findOne({ author: 'albert' });

        assert.isTrue(
          typeof mariesPost !== 'undefined',
          'Marie\'s post present'
        );
        assert.isTrue(
          typeof albertsPost !== 'undefined',
          'Albert\'s post present'
        );
  ...
  testPublication(
    'Should publish two top level publications specifed with an array',
    {
      publication: 'twoFixedAuthors',

      testHandler: function(assert, onComplete) {
        let marie = Authors.findOne({ username: 'marie' });
        let albert = Authors.findOne({ username: 'albert' });
  ...
  testPublication(
    'Should gracefully return if publication handler returns nothing',
    {
  ...
  Meteor.methods({
    initTestData: (function() {
      ...
      function insertPost(title, author, comments) {
        let postId = new Mongo.ObjectID();
        let commentId, commentData;
...
Meteor.methods({
  ...
  updatePostAuthor: function(postId, newAuthor) {
    console.log(
      'calling updatePostAuthor, postId: ' +
        postId +
        ', newAuthor: ' +
        newAuthor
    );
  ...
  updateCommentAuthor: function(commentId, newAuthor) {
    console.log(
      'calling updateCommentAuthor, commentId: ' +
        commentId +
        ', newAuthor: ' +
        newAuthor
    );
