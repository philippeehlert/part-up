...
class _FileController {
  constructor(config = {}) {
    ...
    this.imagesRemaining = new ReactiveVar(
      this.limit.images,
      (oldVal, newVal) => calculateLimit('documentsRemaining', newVal)
    );
    this.documentsRemaining = new ReactiveVar(
      this.limit.documents,
      (oldVal, newVal) => calculateLimit('imagesRemaining', newVal)
    );
    ...
    this.files = new ReactiveVar([], (oldVal, newVal) => {
      ...
      if (newVal) {
        this.imagesRemaining.set(
          this.limit.images -
            _.filter(newVal, (file) => Partup.helpers.files.isImage(file))
              .length
        );
        ...
        if (this.canAdd(file).length) {
          ...
          if (!_.includes(allowedServices, file.service)) {
            return reject({
              ...baseError,
              code: 1,
              message: `this file has an invalid file service: '${
                file.service
              }', see 'Partup.helpers.files.FILE_SERVICES' for more information`,
            });
          }
      ...
      if (file) {
        ...
        if (!collection) {
          const foundFile = _.find(
            _.concat(this.files.get(), this.removedFromCache.get()),
            ({ _id }) => _id === fileId
          );
          if (foundFile) {
      ...
      _.each(fileArray, (file) => {
        if (this.canAdd(file).length) {
          ...
          const collection = Partup.helpers.files.isImage(file)
            ? 'images'
            : 'files';
          this.removeFileFromCollection(file._id, collection).catch((error) => {
            throw error;
          });
        }
      });
  ...
  removeFilesFromCache(fileIds) {
    if (fileIds) {
      ...
      const files = _.filter(
        this.files.get(),
        (file) => !_.includes(ids, file._id)
      );
      this.removedFromCache.set(
        _.concat(
          this.removedFromCache.get(),
          _.filter(this.files.get(), (file) => _.includes(ids, file._id))
        )
      );
  ...
  removeAllFilesBesides(fileIds = []) {
    const filesToRemove = _.filter(
      _.concat(this.files.get(), this.removedFromCache.get()),
      (file) => !_.includes(fileIds, file._id)
    );
    ...
    _.each(filesToRemove, (file) => {
      this.removeFileFromCollection(file._id)
        ...
        .catch((error) => {
          throw error;
        }); // sis not important for the user.
    });
  }
