...
Template.dropboxPicker.onRendered(function() {
  ...
  if (!$trigger) {
    throw new Error(
      'dropboxPicker: expected to find a html element with the "data-browse-dropbox" attribute'
    );
  }
  ...
  const pickerCallback = (dropboxFiles) => {
    ...
    const transformedFiles = _.map(
      dropboxFiles,
      Partup.helpers.files.transform.dropbox
    );
  ...
  const open = () => {
    ...
    if (accessToken) {
      ...
      Dropbox.choose({
        ...
        multiselect: true,
        extensions: _.map(
          Partup.helpers.files.extensions.all,
          (ext) => `.${ext}`
        ),
      });
    } else {
      dropboxClient = new Dropbox({
        clientId: $('#dropboxjs').attr('data-app-key'),
      });
      const authUrl = dropboxClient.getAuthenticationUrl(
        `${new URL(window.location).origin}/dropbox/oauth_receiver.html`
      );
