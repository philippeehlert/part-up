Template.UpperTile.onCreated(function() {
  ...
  user.profile.imageObject =
    user.profile.imageObject || Images.findOne({ _id: user.profile.image });
});
...
Template.UpperTile.helpers({
  chat(...args) {
    const { _id, profile: { chats = [] } } = Template.instance().data.user;
    const currentUserId = Meteor.userId();
    return {
      chatInstanceIdWithCurrentUser: () =>
        (Chats.findForUser(currentUserId, { private: true }) || [])
          .map(({ _id }) => _id)
  ...
  similarities(...args) {
    ...
    return {
      partnerInSamePartupsCount: () => {
        return lodash.size(
          lodash.intersection(userPartups, currentUserPartups)
        );
      },
      memberOfSameNetworkCount: () => {
        return lodash.size(
          lodash.intersection(userNetworks, currentUserNetworks)
        );
      },
      supporterOfPartupsCurrentUserIsPartnerOfCount: () => {
        return lodash.size(
          lodash.intersection(userSupporter, currentUserPartups)
        );
      },
    };
  },
