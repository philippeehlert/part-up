...
Partup.client.discover = {
  ...
  composeTribeQueryObject: function() {
    ...
    for (key in this.DEFAULT_TRIBE_QUERY) {
      let value = this.tribe_query.get(key);
      if (!value) continue;
  ...
  resetTribeQuery: function() {
    ...
    this.current_tribe_query = JSON.parse(
      JSON.stringify(this.DEFAULT_TRIBE_QUERY)
    );
  },
  ...
  setPrefill: function(key, value) {
    if (!Partup.client.discover.DEFAULT_QUERY.hasOwnProperty(key)) {
      throw new Error(
        'Discover query key "' +
          key +
          '" is not defined in Partup.client.discover.DEFAULT_QUERY'
      );
    }
  ...
  getCustomPrefill: function() {
    let copy = _customPrefill;
    _customPrefill = {};
    return copy;
  },
  getCustomTribePrefill: function() {
    let copy = _tribe_customPrefill;
