...
(function(global) {
  ...
  function urlencoded_serialize(pairs) {
    ...
    pairs.forEach(function(pair) {
      let name = encodeURIComponent(pair.name);
      let value = encodeURIComponent(pair.value);
      if (!first) output += '&';
      output += name + '=' + value;
      first = false;
    });
  ...
  Object.defineProperties(URLSearchParams.prototype, {
    ...
    delete: {
      value: function(name) {
        for (let i = 0; i < this._list.length; ) {
  ...
  function URL(url, base) {
    ...
    if (base) {
      url = (function() {
        ...
        let baseTag = doc.createElement('base');
        baseTag.href = base;
    ...
    let instance = URLUtils(url || '');

    // Detect for ES5 getter/setter support
    // (an Object.defineProperties polyfill that doesn't support getters/setters may throw)
    let ES5_GET_SET = (function() {
    ...
    let self = ES5_GET_SET ? this : document.createElement('a');

    let query_object = new URLSearchParams(
    ...
    function tidy_instance() {
      let href = instance.href.replace(/#$|\?$|\?(?=#)/g, '');
