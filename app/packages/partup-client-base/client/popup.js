...
Partup.client.popup = {
  ...
  open: function(options, callback) {
    if (!options || !options.id) {
      console.error('a unique options.id must be defined');
    }
    ...
    let totalImages = options.totalImages || 0;
    let imageIndex = options.imageIndex || 0;
    // Open popup
    this.current.set(id);
  ...
  close: function() {
    // Get current popup
    let current = this.current.curValue;
    if (!current) {
      throw '[Partup.client.popup.close] No current popup found';
    }
    ...
    // Save callback
    let callback = this._closeCallback;
