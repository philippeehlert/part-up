...
(function(navigator, window, document, $) {
  ...
  Trumbowyg.prototype = {
    ...
    buildEditor: function() {
      ...
      t.$ed
        ...
        .on('keydown', function(e) {
          ...
          if (e.ctrlKey) {
            t._ctrl = true;
            let k = t.keys[String.fromCharCode(e.which).toUpperCase()];
    ...
    buildBtnPane: function() {
      ...
      let fullscreenCssClass = prefix + 'fullscreen';
      if (t.o.fullscreenable) {
        $liRight.append(
          t.buildRightBtn('fullscreen').on('click', function() {
            t.$box.toggleClass(fullscreenCssClass);
            $('body').toggleClass(
              prefix + 'body-fullscreen',
              t.$box.hasClass(fullscreenCssClass)
            );
      ...
      if (t.o.closable) {
        $liRight.append(
          t.buildRightBtn('close').on('click', function() {
        ...
        $btn = $('<button/>', {
          ...
          mousedown: function() {
            ...
            if (
              ...
              !$(this)
                .parent()
                .hasClass(prefix + 'not-disable')
            ) {
              return false;
            }

            t.execCmd(
              (d ? 'dropdown' : false) || btn.fn || btnName,
              btn.param || btnName
            );
    ...
    semanticCode: function(force, full) {
      ...
      if (t.o.semantic) {
        ...
        if (full) {
          let inlineElementsSelector = t.o.inlineElementsSelector,
          ...
          var wrapInlinesInParagraphsFrom = function($from) {
            if ($from.length !== 0) {
              ...
              let $nextElement = $finalParagraph
    ...
    createLink: function() {
      ...
      t.openModalInsert(
        ...
        function(v) {
          // v is value
          let link = $(['<a href="', v.url, '">', v.text, '</a>'].join(''));
          if (v.title.length > 0) {
    ...
    openModalInsert: function(title, fields, cmd) {
      ...
      $.each(fields, function(fieldName, field) {
        ...
        html +=
          ...
          'input-infos"><span>' +
          (!l
            ? lg[fieldName] ? lg[fieldName] : fieldName
            : lg[l] ? lg[l] : l) +
          '</span></span></label>';
      });
      ...
      return t
        ...
        .on(CONFIRM_EVENT, function() {
          ...
          $.each(fields, function(fieldName, field) {
            let $field = $('input[name="' + fieldName + '"]', $form);
    ...
    saveSelection: function() {
      ...
      if (documentSelection.rangeCount) {
        let savedSelection = (t.selection = documentSelection.getRangeAt(0)),
          range = t.doc.createRange(),
          rangeStart;
        range.selectNodeContents(t.$ed[0]);
        range.setEnd(savedSelection.startContainer, savedSelection.startOffset);
        rangeStart = String(range).length;
        t.metaSelection = {
          start: rangeStart,
          end: rangeStart + String(savedSelection).length,
        };
      }
    },
    restoreSelection: function() {
      ...
      if (metaSelection && metaSelection.start !== metaSelection.end) {
        ...
        while (!stop && (node = nodeStack.pop())) {
          if (node.nodeType === 3) {
            ...
            if (
              !foundStart &&
              metaSelection.start >= charIndex &&
              metaSelection.start <= nextCharIndex
            ) {
            ...
            if (
              foundStart &&
              metaSelection.end >= charIndex &&
              metaSelection.end <= nextCharIndex
            ) {
