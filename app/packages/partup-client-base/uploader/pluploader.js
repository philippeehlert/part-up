...
class _Pluploader {
  constructor(options) {
    const config = _.extend(
      {
        url: `${
          Partup.client.window.location
        }/files/upload?token=${Accounts._storedLoginToken()}`,
  ...
  setMimeFilters(categories) {
    ...
    if (!categories) {
      filters.mime_types = [
        Partup.helpers.files.toUploadFilter(
          'all',
          Partup.helpers.files.extensions.all
        ),
      ];
    } else {
      filters.mime_types = _.map(categories, (category) =>
        Partup.helpers.files.toUploadFilter(category)
      );
    }
  ...
  addMimeFilter(category) {
    ...
    filters.mime_types.push({
      title: category,
      extensions: Partup.helpers.files.extensions[category],
    });
    this.setOption('filters', filters);
  }
