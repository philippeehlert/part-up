...
JsonRoutes.Middleware.use(function(request, response, next) {
  let urlRegexesToCacheForAnHour = [
  ...
  let imagesAndFonts = [/^.*\.woff(2)*$/, /^.*\.(jpeg|jpg|gif|png)$/];

  let meteorResourceIndicators = [
    'meteor_js_resource=true',
    'meteor_css_resource=true',
  ];

  let cacheControl = 'no-store, max-age=0';
  ...
  let uri = url.parse(request.url, true, true);

  if (
    uri.query.meteor_js_resource ||
    uri.query.meteor_css_resource ||
    uri.query.hash ||
    uri.query.v ||
    imagesAndFonts.some(function(regex) {
