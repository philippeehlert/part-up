  ...
  if (process.env.NODE_ENV.match(/development|staging/)) {
    if (!Updates.find().count()) {
      ...
      for (let hour = 10; hour < 22; hour++) {
        for (let minute = 10; minute < 22; minute++) {
          let comments = [];
          var last_comment_time;
          for (let i = 1; i <= 3; i++) {
            let comment_time = new Date(
              '2015-07-21T' + hour + ':' + (minute + i) + ':20.000Z'
            );
          ...
          Updates.insert({
            ...
            upper_id: 'K5c5M4Pbdg3B82wQH',
            created_at: new Date(
              '2015-07-21T' + hour + ':' + minute + ':20.000Z'
            ),
        ...
        comments: [
          {
            ...
            content:
              'Im known with these kind of platforms, it would be a perfect fit',
            type: 'motivation',
            creator: {
              _id: 'K5c5M4Pbdg3B82wQI',
              name: 'John Partup',
