...
let updateChildren = [
  {
    find: Meteor.users.findUserForUpdate,
  ...
  {
    find: Contributions.findForUpdate,
    children: [
...
Meteor.publishComposite('updates.one', function(updateId) {
  ...
  return {
    find: function() {
      let updateCursor = Updates.find({ _id: updateId }, { limit: 1 });

      let update = updateCursor.fetch().pop();
      if (!update) return;
 ...
 */
Meteor.publishComposite('updates.from_partup', function(
  partupId,
  parameters,
  accessToken
) {
...
Meteor.publishComposite('updates.new_conversations', function({ dateFrom }) {
  ...
  const partupIds = [...(user.upperOf || []), ...(user.supporterOf || [])];
...
Meteor.publishComposite('updates.new_conversations_count', function({
  dateFrom,
}) {
  const user = Meteor.user();
