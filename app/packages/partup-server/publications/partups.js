...
Meteor.routeComposite('/partups/me', function(request, parameters) {
  ...
  const partupsCursor = Partups.guardedFind(
    user,
    {
      _id: {
        $in: [...(user.upperOf || []), ...(user.supporterOf || [])],
      },
    },
    {
      fields: { _id: 1, name: 1, image: 1, upper_data: 1 },
      sort: { popularity: -1 },
    }
  );
...
Meteor.routeComposite('/partups/start', function(request, parameters) {
  ...
  const filesCursor = Files.find({
    _id: {
      $in: [...(updateFileIds || []), ...(activityFilesIds || [])],
    },
  });

  const additionalImagesCursor = Images.find({
    _id: {
      $in: [...(updateImageIds || []), ...(activityImageIds || [])],
    },
  });
...
Meteor.routeComposite('/partups/updates', function(request, parameters) {
  ...
  if (parameters.query.partupId) {
    partupsCursor = Partups.guardedFind(
      userId,
      { _id: { $in: [parameters.query.partupId] } },
      { fields: partupFields }
    );
  } else {
    partupsCursor = Partups.findPartupsIdsForUser(
      user,
      {
  ...
  const userIds = Array.from(
    new Set([...(partupUniqueUpperIds || []), ...(updateUserIds || [])])
  );
...
Meteor.routeComposite('/partups/recommendations', function(
  ...
  if (!Api.available) {
    console.log(
      'WARNING: Api connection not available. For now returning dummy local recommendations'
    );
    partupIds = dummyLocalData.partupIds;
  } else {
    let result = Api.get('/partups/recommended/for/user/' + userId);
    partupIds = result.data.partUpIds;
  }

  return {
    find: function() {
      return Partups.guardedFind(this.userId, {
        _id: { $in: partupIds },
      });
    },
    children: [
...
Meteor.publishComposite(
  ...
  function(language) {
    ...
    return {
      find: function() {
        return Partups.findForDiscover(
          this.userId,
          { limit: 4 },
          { sort: 'popular' }
        );
