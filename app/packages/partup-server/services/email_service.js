    ...
    Meteor.defer(function() {
      ...
      if (!options.type) {
        throw new Meteor.Error(
          'Required argument [options.type] is missing for method [Partup.server.services.emails::send]'
        );
      }
      if (!options.typeData) {
        throw new Meteor.Error(
          'Required argument [options.typeData] is missing for method [Partup.server.services.emails::send]'
        );
      }
      if (!options.toAddress) {
        throw new Meteor.Error(
          'Required argument [options.toAddress] is missing for method [Partup.server.services.emails::send]'
        );
      }
      if (!options.fromAddress) {
        options.fromAddress = Partup.constants.EMAIL_FROM;
      }
      if (!options.subject) {
        throw new Meteor.Error(
          'Required argument [options.subject] is missing for method [Partup.server.services.emails::send]'
        );
      }
      if (!options.locale) {
        throw new Meteor.Error(
          'Required argument [options.locale] is missing for method [Partup.server.services.emails::send]'
        );
      }

      // Check if user has disabled this email type
      if (
        options.userEmailPreferences &&
        !options.userEmailPreferences[options.type]
      ) {
      ...
      let template = 'email-' + options.type + '-' + options.locale;
