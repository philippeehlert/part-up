Meteor.methods({
  ...
  'uploads.parse_csv': function(fileId) {
    ...
    let file = Temp.findOne({ _id: fileId });
    let filePath =
      process.env.TEMP_DIR + '/temp-' + file._id + '-' + file.original.name;

    let fs = Npm.require('fs');

    let emailAddresses = Meteor.wrapAsync(function(filePath, done) {
      fs.readFile(
        filePath,
        'utf8',
        Meteor.bindEnvironment(function(err, fileContent) {
          if (err) {
            ...
            .to.array(
              Meteor.bindEnvironment(function(array) {
                let list = lodash
                  .chain(array)
                  .map(function(row) {
                    if (!Partup.services.validators.email.test(row[1])) {
                      return false;
                    }
