Meteor.methods({
  ...
  'settings.update': function(data) {
    ...
    try {
      let settings = _.extend(upper.profile.settings, data);
  ...
  'settings.update_email_notifications': function(subscriptionKey, newValue) {
    ...
    let emailSubscriptions = user.profile.settings.email;
    if (!emailSubscriptions.hasOwnProperty(subscriptionKey)) {
      throw new Meteor.Error(400, 'invalid_subscription');
    }
    ...
    Meteor.users.update(user._id, {
      $set: { 'profile.settings.email': emailSubscriptions },
    });
  },
  ...
  'settings.email_unsubscribe_one': function(token, subscriptionKey) {
    let user = Meteor.users.findByUnsubscribeEmailToken(token).fetch()[0];
    if (!user) throw new Meteor.Error(401, 'unauthorized');

    let emailSubscriptions = user.profile.settings.email;
    if (!emailSubscriptions) {
      Log.debug('No subscriptions for user ' + user._id);
    }
    if (!emailSubscriptions.hasOwnProperty(subscriptionKey)) {
      throw new Meteor.Error(400, 'invalid_subscription');
    }
  ...
  'settings.email_unsubscribe_all': function(token) {
    ...
    for (let key in emailSubscriptions) {
