Meteor.methods({
  ...
  'meurs.create_test': function() {
    ...
    Meteor.users.update(upper._id, {
      $set: { 'profile.meurs.initiating_test': true },
    });
    ...
    if (!upper.profile.meurs || !upper.profile.meurs.portal) {
      Meteor.users.update(upper._id, {
        $set: { 'profile.meurs.portal': portal },
      });
    }
    ...
    // Authenticate
    let token = Partup.server.services.meurs.getToken(portal);
    ...
    if (
      ...
      // Add user
      q4youId = Partup.server.services.meurs.addUser(
        token,
        upper._id,
        User(upper).getEmail()
      );
      ...
      // Activate user
      let isUserActivated = Partup.server.services.meurs.activateUser(
        token,
        q4youId
      );
      ...
      if (portal === 'en') {
        Meteor.users.update(upper._id, {
          $set: { 'profile.meurs.en_id': q4youId },
        });
      } else if (portal === 'nl') {
        Meteor.users.update(upper._id, {
          $set: { 'profile.meurs.nl_id': q4youId },
        });
      }
    }
    ...
    if (!upper.profile.meurs || !upper.profile.meurs.program_session_id) {
      // Create session
      let programSessionId = Partup.server.services.meurs.createProgramSessionId(
        portal,
        token,
        q4youId
      );
      ...
      let isProgramSessionActivated = Partup.server.services.meurs.setActiveProgramSession(
        token,
        q4youId,
        programSessionId
      );
      ...
      // Update user
      Meteor.users.update(upper._id, {
        $set: { 'profile.meurs.program_session_id': programSessionId },
      });
    }

    // Create return URL
    let returnUrl =
      Meteor.absoluteUrl() + 'profile/' + upper._id + '/?results_ready=true';

    // Generate browser token and return generated URL to FE
    let testUrl = Partup.server.services.meurs.getBrowserToken(
      portal,
      token,
      q4youId,
      returnUrl
    ...
    // Unset loading state
    Meteor.users.update(upper._id, {
      $unset: { 'profile.meurs.initiating_test': '' },
    });
  ...
  'meurs.get_results': function(upperId) {
    ...
    // Get Upper
    let upper = Meteor.users.findOne({ _id: upperId });
    ...
    if (upper.profile.meurs.portal === 'en' && upper.profile.meurs.en_id) {
      ...
      upper.profile.meurs.portal === 'nl' &&
      upper.profile.meurs.nl_id
    ) {
      q4youId = upper.profile.meurs.nl_id;
    }

    // Authenticate
    let token = Partup.server.services.meurs.getToken(
      upper.profile.meurs.portal
    );

    // Get service session ID
    let serviceSessionData = Partup.server.services.meurs.getServiceSessionData(
      token,
      q4youId,
      upper.profile.meurs.program_session_id
    );
    ...
    if (!serviceSessionData || serviceSessionData.serviceSessionStatus !== 2) {
      return false;
    }

    // Get results
    let results = Partup.server.services.meurs.getResults(
      token,
      serviceSessionData.serviceSessionId
    );
    // Order results by score and only store the best 2
    let orderedResults = lodash
      .sortBy(results, function(category) {
    ...
    Meteor.users.update(
      ...
      {
        $set: {
          'profile.meurs.results': orderedResults,
          'profile.meurs.fetched_results': true,
        },
      }
    );
