Meteor.methods({
  ...
  'swarms.update': function(swarmId, fields) {
    ...
    let user = Meteor.user();
    let swarm = Swarms.findOneOrFail(swarmId);
    ...
    try {
      let newSwarmFields = Partup.transformers.swarm.fromFormSwarm(fields);
  ...
  'swarms.admin_update': function(swarmSlug, fields) {
    ...
    let swarm = Swarms.findOneOrFail({ slug: swarmSlug });
  ...
  'swarms.add_network': function(swarmId, networkId) {
    ...
    let hasAccess = User(user).isSwarmAdmin(swarmId) || User(user).isAdmin();
    if (!hasAccess) throw new Meteor.Error(401, 'unauthorized');

    try {
      // Make sure the swarm and network exist
      let swarm = Swarms.findOneOrFail({ _id: swarmId });
      let network = Networks.findOneOrFail({ _id: networkId });
  ...
  'swarms.increase_email_share_count': function(swarmId) {
    ...
    try {
      ...
      throw new Meteor.Error(
        400,
        'swarm_email_share_count_could_not_be_updated'
      );
    }
  },
  ...
  'swarms.add_quote': function(swarmId, fields) {
    ...
    let swarm = Swarms.findOneOrFail(swarmId, { fields: { quotes: 1 } });
    if (swarm.quotes.length >= 3) {
      throw new Meteor.Error(400, 'swarm_quote_limit_reached');
    }

    try {
      let upper = Meteor.users.findOne(fields.author_id);
      let quote = {
        _id: Random.id(),
  ...
  'swarms.update_quote': function(swarmId, quoteId, fields) {
    ...
    try {
      ...
      Swarms.update(
        { '_id': swarmId, 'quotes._id': quoteId },
        {
          $set: {
            'quotes.$.author': {
              _id: upper._id,
  ...
  'swarms.get_related_uppers': function(swarmSlug, amount) {
    ...
    try {
      let swarm = Swarms.findOneOrFail(
        { slug: swarmSlug },
        { fields: { networks: 1 } }
      );
      let swarm_networks = swarm.networks || [];
      let swarm_uppers = [];
      ...
      swarm_networks.forEach(function(networkId) {
        ...
        swarm_uppers.push(...network_uppers);
      });
      ...
      if (!upper) {
        ...
        upper_partups.forEach(function(partupId) {
          ...
          upper_partup_uppers.push(...partup_uppers);
        });
      ...
      for (let i = 0; i < amount; i++) {
        let random_index = Math.floor(Math.random() * upper_list.length);
      ...
      let response = [];
      Meteor.users
        .find({ _id: { $in: related_uppers } })
        .fetch()
        .forEach(function(upper) {
          response.push({
            _id: upper._id,
            image: upper.profile.image,
          });
  ...
  'swarms.get_related_networks': function(swarmSlug) {
    ...
    try {
      ...
      let upper_tags = upper.tags || [];

      // Loop through networks to sort by common tags
      return swarm_networks.sort(function(networkId) {
        let network = Networks.findOne(networkId);
        let network_tags = network.tags || [];
