    ...
    job: function() {
      if (!process.env.NODE_ENV.match(/development/)) {
        ...
        Networks.find(
          ...
          { fields: { slug: 1, updated_at: 1 } }
        ).forEach(function(network) {
          networkXml += '<url>';
          networkXml += '<loc>' + baseUrl + 'tribes/' + network.slug + '</loc>';
          networkXml +=
            '<lastmod>' + network.updated_at.toISOString() + '</lastmod>';
        ...
        let profilesXml =
          '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">';
        Meteor.users
          .find({ deactivatedAt: { $exists: false } }, { fields: { _id: 1 } })
        ...
        let swarmsXml =
          '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">';
        Swarms.find({}, { fields: { slug: 1, updated_at: 1 } }).forEach(
          function(swarm) {
            swarmsXml += '<url>';
            swarmsXml += '<loc>' + baseUrl + swarm.slug + '</loc>';
            swarmsXml +=
              '<lastmod>' + swarm.updated_at.toISOString() + '</lastmod>';
