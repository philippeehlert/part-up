...
Event.on('chats.messages.inserted', function(upper, chatMessage, network) {
  ...
  if (network) {
    ...
    let limitExceeded = Partup.helpers.mentions.exceedsLimit(
      chatMessage.content
    );
    let mentions = Partup.helpers.mentions.extract(chatMessage.content);
    let notificationType = 'upper_mentioned_in_network_chat';
