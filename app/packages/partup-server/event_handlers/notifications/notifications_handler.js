...
Event.on('partups.notifications.inserted', function(upperId, notification) {
  ...
  let conversationGroup = ['partups_new_comment_in_involved_conversation'];
  let networkGroup = ['partups_networks_new_upper'];

  // Check if notification belongs to a group
  let notificationGroupType = null;
  let group = [];
  ...
  if (notificationGroupType) {
    ...
    if (notification.type_data.supporter) {
      latestUpper = notification.type_data.supporter;
    }
    if (notification.type_data.creator) {
      latestUpper = notification.type_data.creator;
    }
    if (notification.type_data.commenter) {
      latestUpper = notification.type_data.commenter;
    }
    if (notification.type_data.upper) {
      latestUpper = notification.type_data.upper;
    }
    ...
    if (group === conversationGroup) {
      query['type_data.update._id'] = notification.type_data.update._id;
    }
    if (group === updateGroup) {
      query['type_data.partup._id'] = notification.type_data.partup._id;
    }
    if (group === networkGroup) {
      query['type_data.network._id'] = notification.type_data.network._id;
    }

    let unreadNotifications = Notifications.find(query, {
      fields: { _id: 1, type_data: 1 },
    }).fetch();
    let notificationIds = [];
    let upperIds = [];
    unreadNotifications.forEach(function(notification) {
      if (notification.type_data.supporter) {
        upperIds.push(notification.type_data.supporter._id);
      }
      if (notification.type_data.creator) {
        upperIds.push(notification.type_data.creator._id);
      }
      if (notification.type_data.commenter) {
        upperIds.push(notification.type_data.commenter._id);
      }
      if (notification.type_data.upper) {
        upperIds.push(notification.type_data.upper._id);
      }
    ...
    let groupNotification = Notifications.findOne(query);
    if (groupNotification) {
      ...
      newNotification.type_data.others_count =
        lodash.unique(upperIds).length - 1; // Don't include latest upper
