Event.on('partups.inserted', function(userId, partup) {
  ...
  let update_created = Partup.factories.updatesFactory.make(
    userId,
    partup._id,
    'partups_created',
    {}
  ...
  let update_systemmessage = Partup.factories.updatesFactory.makeSystem(
    partup._id,
  ...
  if (partup.network_id) {
    ...
    let creator = Meteor.users.findOneOrFail(userId);
    let recipients = network.admins; // Always send to admins
    ...
    lodash.uniq(recipients || []).forEach(function(upperId) {
      ...
      let upper = Meteor.users.findOne(upperId);
      ...
      let emailOptions = {
        ...
        subject: TAPi18n.__(
          'emails-partup_created_in_network-subject',
          { network: network.name },
          User(upper).getLocale()
        ),
        ...
        typeData: {
          ...
          unsubscribeOneUrl:
            Meteor.absoluteUrl() +
            'unsubscribe-email-one/partup_created_in_network/' +
            upper.profile.settings.unsubscribe_email_token,
          unsubscribeAllUrl:
            Meteor.absoluteUrl() +
...
Event.on('partups.archived', function(userId, partup) {
  ...
  let update_archived = Partup.factories.updatesFactory.make(
    userId,
    partup._id,
    'partups_archived',
    {}
  ...
  let archiver = Meteor.users.findOneOrFail(userId);

  let notify_uppers = partup.uppers || [];
  let partup_supporters = partup.supporters || [];
  notify_uppers.push(...partup_supporters);
...
Event.on('partups.unarchived', function(userId, partup) {
  ...
  let update_unarchived = Partup.factories.updatesFactory.make(
    userId,
    partup._id,
    'partups_unarchived',
    {}
  ...
  let unarchiver = Meteor.users.findOneOrFail(userId);
...
Event.on('partups.privacy_type_changed', function(
  userId,
  partup,
  oldPrivacyType,
  newPrivacyType
  ...
  if (
    oldPrivacyType == Partups.privacy_types.NETWORK_PUBLIC ||
    ...
    newPrivacyType == Partups.privacy_types.NETWORK_PUBLIC ||
    ...
    if (newPrivacyType == Partups.privacy_types.NETWORK_COLLEAGUES_CUSTOM_B) {
      ...
      newPrivacyType == Partups.privacy_types.NETWORK_COLLEAGUES_CUSTOM_A
    ) {
      // Add custom A uppers
      recipients.push(...network.colleagues_custom_a);
